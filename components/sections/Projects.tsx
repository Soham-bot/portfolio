      ...
      <article className="project-card border border-[var(--border)] bg-[var(--surface)] p-6 hover:border-[var(--accent)]/20 relative group flex flex-col">
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          {project.verification === "verified" && (
            <span className="font-mono text-[10px] text-[var(--success)]/60">● verified</span>
          )}
        </div>
        ...
        <p className="font-mono text-xs text-[var(--accent)]/60 mb-3">{project.category}</p>
        ...
        <div className="flex flex-wrap gap-1 mb-5">
          {project.tech.slice(0, 6).map((t) => (
            <span key={t} className="font-mono text-[10px] bg-[var(--bg)] border border-[var(--border)] text-[var(--text-muted)] px-2 py-0.5">{t}</span>
          ))}
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
          {project.github && (
            <a href={project.github} target="_blank" rel="noopener noreferrer"
              className="font-mono text-xs flex items-center gap-1.5 text-[var(--text-muted)] border border-[var(--border)] px-3 py-1.5 hover:text-[var(--accent)] hover:border-[var(--accent)]/30 transition-all">
              <Github size={12} /> GitHub
            </a>
          )}
          {project.live && (
            <a href={project.live} target="_blank" rel="noopener noreferrer"
              className="font-mono text-xs flex items-center gap-1.5 text-[var(--accent)] border border-[var(--accent)]/30 px-3 py-1.5 hover:bg-[var(--accent-dim)] transition-all">
              <ExternalLink size={12} /> Live Demo
            </a>
          )}
          {project.architecture && (
            <button onClick={() => setShowArch(true)}
              className="font-mono text-xs flex items-center gap-1.5 text-[var(--text-dim)] border border-[var(--border)] px-3 py-1.5 hover:text-[var(--accent)] hover:border-[var(--accent)]/30 transition-all">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <p className="text-[var(--text-muted)] font-mono text-sm max-w-xl">
            <span className="text-[var(--accent)]">//</span> Ranked by engineering complexity and real implementation depth.
          </p>
