      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="border border-[var(--border)] bg-[var(--surface)] p-6">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={stat.label} className="flex justify-between items-center border-b border-[var(--border)] pb-2">
                  <span className="font-mono text-xs text-[var(--text-muted)]">{stat.label}</span>
                  <span className="font-mono text-xs text-[var(--text)]">{stat.value}</span>
                </div>
              ))}
            </div>
            <a href={PERSONAL.github} target="_blank" rel="noopener noreferrer"
              className="mt-6 w-full font-mono text-xs border border-[var(--accent)]/30 text-[var(--accent)] px-4 py-2 hover:bg-[var(--accent-dim)] transition-all flex items-center justify-center gap-2">
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="font-mono text-[10px] text-[var(--text-muted)] mb-4">SELECTED REPOSITORIES</div>
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  {repo.live && (
                    <a href={repo.live} target="_blank" rel="noopener noreferrer"
                      className="font-mono text-[10px] text-[var(--success)]/60 hover:text-[var(--success)] border border-[var(--success)]/20 px-2 py-0.5">
                      LIVE ↗
                    </a>
                  )}
                </div>
                <p className="text-[var(--text-muted)] text-xs mb-3">{repo.desc}</p>
                <div className="flex items-center gap-3">
                  {repo.langs.map((lang) => (
                    <div key={lang} className="flex items-center gap-1">
                      <div className="w-2 h-2 rounded-full bg-[var(--accent)]/40" />
                      <span className="font-mono text-[10px] text-[var(--text-muted)]">{lang}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <a href={`${PERSONAL.github}?tab=repositories`} target="_blank" rel="noopener noreferrer"
              className="block border border-dashed border-[var(--border)] p-4 text-center hover:border-[var(--accent)]/20 transition-all group">
              <span className="font-mono text-xs text-[var(--text-subtle)] group-hover:text-[var(--accent)] transition-colors">
